...
export default function MessagesPage() {
    ...
    const [selectedContactId, setSelectedContactId] = useState("test-freelancer-id"); // Hardcoded for demo
    ...
    const fetchMessages = async () => {
        try {
            ...
            if (Array.isArray(data)) {
                setMessages(data); // Expecting ascending order
            } else {
                setMessages([]);
            }
        } catch (err) {
        ...
        <div className="h-[calc(100vh-14rem)] flex gap-8">
            ...
            >
                <div className="p-8 border-b border-border/30 bg-primary/5">
                    <h3 className="font-black text-xs uppercase tracking-[0.3em] text-primary mb-1">Signal Hub</h3>
                    <p className="text-xl font-black text-foreground/80 tracking-tighter">Contact Grid</p>
                </div>
                    ...
                    {[1, 2, 3].map(i => (
                        ...
                        >
                            <div className="relative">
                                <div className="h-12 w-12 rounded-xl bg-primary/20 flex items-center justify-center text-primary font-black shadow-lg">U{i}</div>
                                <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-green-500 border-2 border-background rounded-full" />
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-center mb-0.5">
                                    <p className="font-black text-sm text-foreground truncate uppercase tracking-tighter">Freelancer {i}</p>
                                    <span className="text-[9px] font-bold text-muted-foreground uppercase">{i === 1 ? 'Now' : i + 'H'}</span>
                                </div>
                                <p className="text-[11px] text-muted-foreground truncate font-medium italic">Project files ready for review...</p>
                            </div>
                        </motion.div>
                    ))}
                </div>
            ...
            >
                ...
                <div className="p-8 border-b border-border/30 flex items-center justify-between backdrop-blur-md bg-white/5">
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            <h3 className="text-xl font-black tracking-tighter uppercase">Deployment Stream</h3>
                            <div className="flex items-center gap-2">
                                <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                                <p className="text-[10px] font-black text-green-500 uppercase tracking-[0.2em]">Secured Link</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar bg-white/2 overflow-x-hidden">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center h-full space-y-4">
                            ...
                            <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Retrieving data stream...</p>
                        </div>
                        ...
                        <div className="flex flex-col items-center justify-center h-full space-y-6 opacity-30">
                            ...
                            <p className="text-xs font-black uppercase tracking-[0.3em] text-muted-foreground">Operational silence detected.</p>
                        </div>
                    ) : (
                        <AnimatePresence initial={false}>
                            {messages.map((msg, idx) => (
                                ...
                                >
                                    <div className="max-w-[80%] flex flex-col items-start text-left">
                                        <div className="glass-morphism bg-white p-5 rounded-[1.8rem] rounded-tl-[0.3rem] shadow-xl relative overflow-hidden transition-all border border-white/20">
                                            <p className="text-sm font-medium leading-relaxed text-foreground/80">{msg.message}</p>
                                        </div>
